...
export function CharadesSettingsOverlay({
  ...
  const [pendingExitView, setPendingExitView] = useState<CharadesMenuView | null>(null)
  ...
  useEffect(() => {
    registerSettingsExitGuard?.((nextView: CharadesMenuView) => {
  ...
  useEffect(() => {
    ...
    if (focusedArea === 'tabs') {
      hostNavigation.setFocus({
        ...
        targetId: getCharadesSettingsTabTarget(activeCategory.id),
      })
      return
    }
    ...
    if (focusedArea === 'device') {
      hostNavigation.setFocus({
        ...
        targetId: getCharadesControlsDeviceTarget(activeControlsDevice),
      })
      return
    }
    ...
    if (focusedArea === 'bindings') {
      hostNavigation.setFocus({
        ...
        targetId: activeBindingId || visibleBindings[0]?.id || CHARADES_NAVIGATION_TARGETS.settingsFooterReset,
      })
      return
    }
    ...
    const entryTarget = getCharadesSettingsEntryTarget(activeCategory.id)
    hostNavigation.setFocus({
      screenId: CHARADES_NAVIGATION_SCREENS.settings,
  ...
  function stepCategory(direction: -1 | 1) {
    ...
    const nextCategory = charadesSettingsCategories.find((category) => category.id === nextCategoryId)
    if (nextCategory) {
  ...
  function commitExitView(view: CharadesMenuView) {
  ...
  function requestSettingsExit(nextView: CharadesMenuView) {
    setListeningBindingKey(null)
    ...
    onAction: (action, input) => {
      ...
      if (focusedArea === 'tabs') {
        ...
        if (tabsCommand?.type === 'select-category') {
          const nextCategory = charadesSettingsCategories.find((category) => category.id === tabsCommand.categoryId)
          if (nextCategory) {
            handleCategoryChange(nextCategory)
          }
          return
