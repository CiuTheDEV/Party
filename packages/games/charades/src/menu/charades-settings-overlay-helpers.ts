...
const CHARADES_SETTINGS_DIRTY_STORAGE_KEY = 'charades:settings:dirty'
...
export function publishSettingsDirtyState(
  ...
  if (typeof window !== 'undefined') {
    if (isDirty) {
      window.sessionStorage.setItem(CHARADES_SETTINGS_DIRTY_STORAGE_KEY, '1')
    } else {
      window.sessionStorage.removeItem(CHARADES_SETTINGS_DIRTY_STORAGE_KEY)
    }
  }
