import { codenamesSettingsCategories, type CodenamesControlsBinding, type CodenamesControlsDevice } from './codenames-settings-overlay-data'

export const CODENAMES_BINDINGS_STORAGE_KEY = 'codenames:settings:bindings:v1'
export const CODENAMES_BINDINGS_UPDATED_EVENT = 'codenames:settings:bindings-updated'
...
export function hasBindingChanges(savedBindings: Record<string, string>, draftBindings: Record<string, string>) {
  ...
  for (const key of allKeys) {
    if ((savedBindings[key] ?? '') !== (draftBindings[key] ?? '')) return true
  }
  return false
}
...
export function detectGamepadProfile(gamepadId: string): GamepadProfile {
  ...
  if (normalizedId.includes('xbox') || normalizedId.includes('xinput') || normalizedId.includes('microsoft')) return 'xbox'
  if (
...
export function loadPersistedBindings() {
  if (typeof window === 'undefined') return createDefaultBindings()

  const defaults = createDefaultBindings()
  const raw = window.localStorage.getItem(CODENAMES_BINDINGS_STORAGE_KEY)
...
export function persistBindings(bindings: Record<string, string>) {
  ...
  window.localStorage.setItem(CODENAMES_BINDINGS_STORAGE_KEY, JSON.stringify(bindings))
  window.dispatchEvent(new CustomEvent(CODENAMES_BINDINGS_UPDATED_EVENT, { detail: bindings }))
}

export function normalizeKeyboardInput(key: string) {
  if (ignoredKeyboardKeys.has(key)) return null
  if (keyboardLabelMap[key]) return keyboardLabelMap[key]
...
export function getGamepadInputLabel(gamepad: Gamepad, previousSnapshot?: GamepadSnapshot | null) {
  ...
  if (pressedButton >= 0) return controllerButtonLabels[pressedButton] ?? `Button ${pressedButton + 1}`
...
export function applyBindingAssignment(
  ...
  targetBinding: CodenamesControlsBinding,
...
export function getBindingDevice(bindingId: string): CodenamesControlsDevice | null {
