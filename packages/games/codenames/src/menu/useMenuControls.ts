...
import type { HostControlAction, HostControlDevice } from './menu-controls'
...
export function useMenuControls({
  ...
  useHostNavigationInput({
    ...
    onAction: (action, input) => {
      ...
      if (!inputLabel || !isHostControlAction(action)) return
...
function reportDevice(
  ...
  if (lastDeviceRef.current === nextDevice) return
